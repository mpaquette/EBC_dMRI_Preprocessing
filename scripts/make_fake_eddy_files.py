...
def buildArgsParser():
    ...
    p.add_argument('--Ndir', dest='Ndir', action='store', type=str,
                            help='bval file (for Number of Diffusion Directions)')
...
def main():
    ...
    TE = args.TE
    N = np.genfromtxt(args.Ndir).shape[-1]
    PE = args.PE

    if int(np.abs(PE)) != 1:
        print('Change PE direction to 1 or -1')

    print('Making Eddy files for {} directions'.format(N))
