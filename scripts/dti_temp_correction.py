...
def main():
    ...
    img = nib.load('diff/data/data_debias_denoise_degibbs_driftcorr.nii.gz')
    data = img.get_fdata()

    # load bval bvec
    # bval, bvec = read_bvals_bvecs(args.bval, args.bvec)
    bval, bvec = read_bvals_bvecs('diff/data/data.bval', 'diff/data/data.bvec')
    ...
    # index = np.genfromtxt(args.index).astype(np.bool).ravel()
    index = np.genfromtxt('config/temp_corr_index.txt').astype(np.bool).ravel()
    if index.shape[0] != bval.shape[0]:
        print('index length different from bval length ({} vs {})'.format(index.shape[0], bval.shape[0]))
        return None 


    # check that index comtains at least 1 b0
    ...
    mask = nib.load('diff/data/mask_ero_for_temp.nii.gz').get_fdata().astype(bool)
    ...
    np.savetxt('diff/data/computed_temp_diff_mult.txt', ks);
    ...
    nib.nifti1.Nifti1Image(data_corr.astype(np.float32), img.affine).to_filename('diff/data/data_debias_denoise_degibbs_driftcorr_detrend.nii.gz')
    ...
    nib.nifti1.Nifti1Image(signal_mult, img.affine).to_filename('diff/data/computed_temp_signal_mult.nii.gz')
