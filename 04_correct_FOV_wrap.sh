...
#
# Previous Steps:
# - Setting $CHECK_REORIENT_SCAN in SET_VARIABLES.sh
# - Setting $NOISE_SCAN          in SET_VARIABLES.sh
# - Setting $TOPUP_LR_RUN        in SET_VARIABLES.sh
# - Setting $TOPUP_RL_RUN        in SET_VARIABLES.sh
# - Setting $DIFF_SCANS          in SET_VARIABLES.sh
# - Setting $FLASH_FA_05         in SET_VARIABLES.sh
# - Setting $FLASH_FA_12p5       in SET_VARIABLES.sh
# - Setting $FLASH_FA_25         in SET_VARIABLES.sh
# - Setting $FLASH_FA_50         in SET_VARIABLES.sh
# - Setting $FLASH_FA_80         in SET_VARIABLES.sh
# - Setting $FLASH_HIGHRES       in SET_VARIABLES.sh
# - Setting $FLASH_ULTRA_HIGHRES in SET_VARIABLES.sh
# - Setting $FOV_MASK_PATHS      in SET_VARIABLES.sh
# - Setting $FOV_OVERLAP_PATHS   in SET_VARIABLES.sh
#
...
echo "mrview -load ${IMAGE_FOR_MASK} -interpolation 0 -load ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz -interpolation 0" >> $THISLOG
...
MASK_THRESHOLD_FLASH=$(${FSL_LOCAL}/fslstats ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz -m)
...
${FSL_LOCAL}/fslmaths \
        ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz \
        -kernel 3D \
        -fmedian \
...
# mrview ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz \
...
echo "mrview ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz -interpolation 0 -overlay.load ${UNWRAP_PROC_DIR}/mask_th_flash.nii.gz -overlay.opacity 0.4 -overlay.colour 1,0,0 -overlay.interpolation 0 -overlay.threshold_min 0.1" >> $THISLOG




bet2 ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz \
...
echo "mrview ${UNWRAP_PROC_DIR}/data_flash_mask_N4_${N4_ITER}x.nii.gz -interpolation 0 -overlay.load ${UNWRAP_PROC_DIR}/flash_bet_mask.nii.gz -overlay.opacity 0.4 -overlay.colour 1,0,0 -overlay.interpolation 0 -overlay.intensity 0,1" >> $THISLOG
...
echo "mrview -load ${IMAGE_FOR_MASK} -interpolation 0 -load ${UNWRAP_PROC_DIR}/data_dwi_N4_${N4_ITER}x.nii.gz -interpolation 0" >> $THISLOG
...
if [ -n "$FLASH_HIGHRES" ]; then
  ...
  echo -e "\necho \"Skipping Flash Highres.\"" >> $THISLOG
fi
...
if [ -n "$FLASH_ULTRA_HIGHRES" ]; then
  ...
  echo -e "\necho \"Skipping Flash Ultra Highres.\"" >> $THISLOG
fi
